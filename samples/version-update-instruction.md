...
0. Final Shape (Canonical Invariant)
- The version block must be normalized to the canonical final shape every time a
  Scala file header is updated.
...
- A valid final version block has:
  1. `@since`, if present
  2. preserved `*  version ...` history lines in original relative order, after
     cleanup rules are applied
...
- A `*  version ...` line in the same month/year as `@since` is valid when the
  day differs from `@since`.
- There must be no `*  version ...` line in the same month/year as the final latest
  `@version`.
- Header repair and normal update use the same final-shape rules. Do not merely
...
- For older "@version" lines:
  - If the older date is in a different month/year from the latest "@version",
    convert it into a history entry:
        *  version <old-date>
  - If the older date is in the same month/year as the final latest "@version",
...
- The final version block must have the canonical order:
  1. `@since`, if present
...
- @since overlap cleanup:
  ...
  - Do not remove a history line merely because it is in the same month/year as
    "@since" when the day differs.
  - Example:
    Before:
        * @since   Mar. 20, 2026
        *  version Mar. 20, 2026
    ...
    After:
        ...
        * @version Mar. 25, 2026
- Month compression:
  - If a line converted from an old `@version` would be in the same month/year as the final latest "@version",
    remove that converted line.
  - If an already-existing `*  version ...` history line is in the same month/year as the final latest "@version",
    remove that history line too.
  - `@version` represents the latest edit in the month; same-month history lines are compressed into it.
  - Exception:
    ...
    - This exception applies only when that preserved history line is not in the
      same month/year as the final latest "@version".
    - Example:
      Before:
          * @since   Mar. 21, 2026
          * @version Mar. 23, 2026
...
- Do NOT remove existing history entries, except when the history line has the same date as `@since`
  or is in the same month/year as the final latest `@version`
